...
QString EncodeGUI::BuildScript(int width, int height, QString jobID) {
	...
	if (CHECKED(ui.InterpolationCB)) {
		...
		int model, thread, num, den, multi, model2, den2;
		...
		QString fpsString2 = QString("%1").arg(inFPS * multi);
		
		den2 = Counter(fpsString);

		if (outFPS == multi * inFPS)
			den = Counter(fpsString);
		else
			den = Counter(fpsString2);

		remv = fpsString.remove(".").remove(",");
		num = remv.toInt();
		...
		if (ui.ToolInterpDD->currentIndex() == 0 || ui.ToolInterpDD->currentIndex() == 1) {
			...
			case 0:
				if (ui.ToolInterpDD->currentIndex() == 1)
					...
					if (model2 == 0)
						ScriptBuilder::SetRIFECuda(id, 4.6, QString("%1").arg(multi * inFPS).remove(".").toInt(), den, scale, sc);
					else if (model2 == 1)
						ScriptBuilder::SetRIFECuda(id, 4.0, QString("%1").arg(multi * inFPS).remove(".").toInt(), den, scale, sc);
				break;
			case 1:
				if (ui.ToolInterpDD->currentIndex() == 1) {
					...
					if (model2 == 0)
						ScriptBuilder::SetRIFENcnnNew(5, id, thread, QString("%1").arg(multi * inFPS).remove(".").toInt(), den, tta, uhd, sc);
					else if (model2 == 1)
			...
			if (ui.ToolInterpDD->currentIndex() == 0 && inFPS * multi != outFPS) {
				...
				if (ui.BackendDD->currentIndex() == 0)
					ScriptBuilder::SetSVPFlowNoob("True", id, 13, 150, 0, num, den2);
				else
					ScriptBuilder::SetSVPFlowNoob("True", id, 13, 0, 0, num, den2);
			}
		}
		else {
			QString useGPU;
...
int EncodeGUI::Counter(QString value) {
	int den = 0;

	if (value.contains(".")) {
		switch (DecimalCounter(value)) {
		case 1:
			den = 10;
			break;
		case 2:

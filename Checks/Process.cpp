...
bool Checks::CheckAudioCompatability(int format, QString container) {
	...
	case AAC:
		...
		break;
	case MP3:
		if (container.contains(".mp4") || container.contains(".mkv") || container.contains(".mov") || container.contains(".avi") || container.contains(".asf") || container.contains(".ts") || container.contains(".flv"))
			result = true;
		else
			result = false;
		break;
	...
	case VORBIS:
		...
		break;
	case OPUS:
		if (container.contains(".mp4") || container.contains(".mkv") || container.contains(".ts") || container.contains(".ogv") || container.contains(".webm"))
			result = true;
		else
			result = false;
		break;
